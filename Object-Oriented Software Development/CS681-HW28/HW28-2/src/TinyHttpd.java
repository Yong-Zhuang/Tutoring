...
//
		switch(fileCacheType)
		{
			case "lfu":
			{
				filecache = new FileCacheLFU(10,accesscounter, lock);
				break;				
			}
			case "lru":
			{
				filecache = new FileCacheLRU(10,accesscounter, lock);
				break;
			}			
		}
	}
		...
		try {
			try {
				...
				while (true) {
					...
					//pool.execute(new runnableExecuteCommand(client));
					pool.execute(new HttpHandler(this,client,serverSocket,this.accesscounter,this.filecache));
